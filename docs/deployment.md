...
Set `DEMO_VOLUME` (see above) so the sandbox is big enough to show the dynamic tables
doing something — with the curated nine vendors a visitor never sees pagination, or a
filter that narrows anything. The generated data is deterministic and regenerated by the
nightly reset, so the demo looks the same every morning.
